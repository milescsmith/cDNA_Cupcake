...
def ensure_pickle_goodness(
    ...
    if a["fasta_filename"] != os.path.abspath(os.path.join(root_dir, "current.fasta")):
        raise Exception(
            "The pickle file {0} indicates that current.fasta is not being used. ICE2 likely did not finish to a point that could be picked up.".format(
                pickle_filename
            )
        )
    ...
    if "root_dir" not in a:
        print(
            "Pickle {0} missing some key-values. Fixing it.".format(pickle_filename),
            file=sys.stderr,
        )
        a["root_dir"] = root_dir
        ...
        with open(pickle_filename + ".fixed", "w") as f:
            dump(a, f)
        print(
            "Fixed pickle written to {0}.fixed".format(pickle_filename), file=sys.stderr
        )
