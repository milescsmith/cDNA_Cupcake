...
t.loci           <- sort(t.loci, decreasing = TRUE)
table.loci_top10 <-
  tableGrob(
    t.loci[1:10],
    rows = rownames(t.loci)[1:10],
    cols = c("count")
  )
grid.arrange(
  p.loci_hist,
...
p.logo.umi.text  <-
  textGrob(
    "Frequency of UMI base at each position",
		gp =
      gpar(
        fontface = "italic",
        fontsize = 12
...
if (any(!is.na(x$BC))) {
  ...
  p.logo.bc.text <-
    textGrob(
      "Frequency of BC base at each position",
      gp = gpar(
        fontface = "italic",
        fontsize = 12
      ),
