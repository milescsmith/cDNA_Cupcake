...
def make_exons_from_base_mapping(mapping, start, end, strand):
    ...
    if strand == "+":
        return [
            Interval(output[i][0], output[i + 1][0] + 1)
...
def get_base_to_base_mapping_from_sam(
    exons, cigar_string, qStart, qEnd, strand, include_junction_info=False
):
    """
    ...
    for num, s in iter_cigar_string(cigar_string):
        if s == "S":  # soft clipping at the ends, ignore
            if start_soft_clip:
                assert num == qStart
                for i in range(num):
                    mapping[i] = (cur_genome_loc, False)
    ...
    if strand == "-":
        mapping = {qLen - 1 - k: v for k, v in mapping.items()}
