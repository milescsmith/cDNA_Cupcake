...
def pick_rep(
    ...
    for r in BioReaders.GMAPSAMReader(sam_filename, True):
        if r.qID in id_to_rep:
            ...
            isoform_index = get_isoform_index(
                fusion_candidate_ranges[r.qID], r.sID, r.sStart, r.sEnd
            )
            ...
            coords[r.qID][
                isoform_index
            ] = f"{r.sID}:{r.sStart}-{r.sEnd}({r.flag.strand})"
    ...
    for pb_id in rep_info:
        ...
        if is_fq:
            SeqIO.write(
                SeqRecord(
                    _seq_, id=_id_, letter_annotations={"phred_quality": best_qual}
                ),
                fout,
                "fastq",
            )
...
def is_fusion_compatible(
    r1, r2, max_fusion_point_dist, max_exon_end_dist, allow_extra_5_exons
):
    """
    ...
    if r1.qStart <= 0.5 * r1.qLen:  # in the 5' portion of r1
        if r2.qStart > 0.5 * r2.qLen:  # in the 3' portion, reject
            return False
        in_5_portion = True
    else:  # in the 3' portion of r1
        if r2.qStart <= 0.5 * r2.qLen:
            return False
        in_5_portion = False
    ...
    if feature == "exact":
        if len(r1.segments) == 1:
            if len(r2.segments) == 1:
                ...
                raise Exception(
                    "Not possible case for multi-exon transcript and "
        ...
        if allow_extra_5_exons:
            ...
            if in_5_portion and plus_is_5end:
                if (
                    abs(r1.segments[-1].start - r2.segments[-1].start)
                    > max_exon_end_dist
                ):
                    return False
                if (
                    abs(r1.segments[-1].end - r2.segments[-1].end)
                    > max_fusion_point_dist
                ):
                    return False
                return True
            elif in_5_portion and (not plus_is_5end):
                if abs(r1.segments[0].end - r2.segments[0].end) > max_exon_end_dist:
                    return False
                if (
                    abs(r1.segments[0].start - r2.segments[0].start)
                    > max_fusion_point_dist
                ):
                    return False
                return True
...
def merge_fusion_exons(
    records, max_fusion_point_dist, max_exon_end_dist, allow_extra_5_exons
):
    """
...
def find_fusion_candidates(
    ...
    """
    TmpRec = namedtuple(
        "TmpRec", ["qCov", "qLen", "qStart", "qEnd", "sStart", "sEnd", "iden", "chrom"]
    )
    ...
    for k, data in d.items():
        if (
            len(data) > 1
            and all(a.iden >= min_identity for a in data)
            and all(a.qCov >= min_locus_coverage for a in data)
            and all(a.qCov * a.qLen >= min_locus_coverage_bp for a in data)
            and total_coverage(data) * 1.0 / data[0].qLen >= min_total_coverage
            and all(
                max(a.sStart, b.sStart) - min(a.sEnd, b.sEnd) >= min_dist_between_loci
...
def fusion_main(
    ...
    for recs in iter_gmap_sam_for_fusion(
        sam_filename, fusion_candidates, bs.transfrag_len_dict
    ):
        for v in recs.values():
            if len(v) > 0:
                o = merge_fusion_exons(
                    v,
    ...
    for qid, indices in compressed_records_pointer_dict.items():
        ...
        for i in indices:
            ...
            isoform_index = get_isoform_index(
                fusion_candidates[qid],
                records[0].sID,
                records[0].sStart,
                records[0].sEnd,
            )
            f_group.write(
                f'"PBfusion".{gene_index}.{isoform_index+1}\t'
                f'{",".join(r.qID for r in records)}\n'
            )
        gene_index += 1
    f_group.close()
    ...
    f_group = open(f"{output_prefix}.group.txt", "w")
    ...
    with open("branch_tmp.group.txt") as f:
        while True:
            ...
            f_group.write(f'{pbid1[: pbid1.rfind(".")]}\t{",".join(group)}\n')
            group_info[pbid1[: pbid1.rfind(".")]] = list(group)
            count += 1
    f_group.close()

    gff_filename = f"{output_prefix}.gff"
    group_filename = f"{output_prefix}.group.txt"
    if is_fq:
        output_filename = f"{output_prefix}.rep.fq"
    else:
        output_filename = f"{output_prefix}.rep.fa"
    pick_rep(
        fa_or_fq_filename,
        sam_filename,
        gff_filename,
