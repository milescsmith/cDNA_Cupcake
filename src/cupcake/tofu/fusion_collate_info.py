...
def collate_info(
    ...
    if config_filename is not None:
        print(f"Reading config file {config_filename}...", file=sys.stdout)
    ...
    f = open(f"{fusion_prefix}.annotated.txt", "w")
    f_bad = open(f"{fusion_prefix}.annotated_ignored.txt", "w")
    ...
    for gene_index, iso_dict in d.items():
        ...
        has_novel = any(
            r["associated_gene"].startswith("novelGene") or r["associated_gene"] == ""
        ...
        rec1 = gff_info[0][1]
        rec2 = gff_info[-1][1]
        left_breakpoint, left_seq, right_breakpoint, right_seq = get_breakpoint_n_seq(
            rec1, rec2, genome_dict
        )
        ...
        if has_novel:
            ...
            left_chr == right_chr
            and abs(int(left_break) - int(right_break)) / 1000 <= min_breakpoint_dist_kb
        ):
...
def main():
    ...
    parser.add_argument(
        "genepred_filename", help="GenePred filename used by SQANTI3 classification"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="(optional) Total FL count used to normalize fusion counts",
    )
    parser.add_argument(
        "--config", help="(optional) Additional information to include in the output"
    )
    parser.add_argument("--genome", help="(optional) Reference genome")
    parser.add_argument(
        "--min_fl_count", type=int, default=2, help="Minimum FL count (default: 2)"
    )
    parser.add_argument(
        "--min_breakpoint_dist_kb",
        type=int,
        default=10,
        help="Minimum breakpoint distance, in kb (default: 10[kb])",
    )
    parser.add_argument(
        "--include_Mt_genes",
        default=False,
        action="store_true",
        help="Include Mt genes (default: off)",
    )
