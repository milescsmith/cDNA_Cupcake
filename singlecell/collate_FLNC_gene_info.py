...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_filename):
        print(
            "Output file {0} already exists. Abort!".format(args.output_filename),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if not os.path.exists(args.group_filename):
        print(
            "Group file {0} not found. Abort!".format(args.group_filename),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if not os.path.exists(args.csv_filename):
        print(
            "CSV file {0} not found. Abort!".format(args.csv_filename), file=sys.stderr
        )
        sys.exit(-1)

    if not os.path.exists(args.class_filename):
        print(
            "Class file {0} not found. Abort!".format(args.class_filename),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if args.ontarget_filename is not None and not os.path.exists(
        ...
        print(
            "Ontarget file {0} given but not found. Abort!".format(
                args.ontarget_filename
            ),
            file=sys.stderr,
        )
    ...
    if args.dedup_ORF_prefix is not None:
        if not os.path.exists(args.dedup_ORF_prefix + ".group.txt"):
            print(
                "Dedup {0}.group.txt not found. Abort!".format(args.dedup_ORF_prefix),
                file=sys.stderr,
            )
            sys.exit(-1)
        if not os.path.exists(args.dedup_ORF_prefix + ".faa"):
            print(
                "Dedup {0}.faa not found. Abort!".format(args.dedup_ORF_prefix),
                file=sys.stderr,
            )
            sys.exit(-1)
