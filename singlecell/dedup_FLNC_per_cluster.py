...
def main(
    ...
    for k in CORRECTED_CSV_FILELDS:
        if k not in reader.fieldnames:
            print(
                "The following fields must exist in {0}!\n{1}".format(
            ...
            for r in SeqIO.parse(open(fasta_file), "fasta"):
                if r.id in per_pbid:
                    ...
                    for c in per_pbid[r.id]["clusters"]:
                        if c not in f_d:
                            f_d[c] = open(
                                "{o}.{c}.fasta".format(o=output_prefix, c=c), "w"
                            )
                        f_d[c].write(">{0}\n{1}\n".format(newid, r.seq))
            ...
            for r in SeqIO.parse(open(faa_file), "fasta"):
                if r.id in per_pbid:
                    ...
                    for c in per_pbid[r.id]["clusters"]:
                        if c not in f_d:
                            f_d[c] = open(
                                "{o}.{c}.faa".format(o=output_prefix, c=c), "w"
                            )
                        f_d[c].write(">{0}\n{1}\n".format(newid, r.seq))
            ...
            for r in collapseGFFReader(gff_file):
                if r.seqid in per_pbid:
                    ...
                    for c in per_pbid[r.seqid]["clusters"]:
                        if c not in f_d:
                            f_d[c] = open(
                                "{o}.{c}.gff".format(o=output_prefix, c=c), "w"
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "corrected_csv", help="Annotated, error-corrected FLNC CSV file"
    )
    ...
    if not os.path.exists(args.corrected_csv):
        print(
            "Input file {0} does not exist! Abort!".format(args.corrected_csv),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if not os.path.exists(args.cluster_file):
        print(
            "Input file {0} does not exist! Abort!".format(args.cluster_file),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if ("cell_barcode" not in reader.fieldnames) or (
        ...
        print(
            "Cluster file {0} must contain 'cell_barcode' and 'cluster' fields!".format(
                args.cluster_file
            ),
            file=sys.stderr,
        )
