...
def make_file_for_subsample(
    ...
    if not include_single_exons:
        ...
        print(
            "Reading {0} to exclude single exons...".format(gff_filename),
            file=sys.stderr,
        )
        good_ids = []
    ...
    if matchAnnot_parsed is not None:
        ...
        print(
            "Reading {0} to get gene/isoform assignment...".format(sqanti_class),
            file=sys.stderr,
        )
        match_dict = {}
        ...
        for pbid in to_write[sample]:
            if matchAnnot_parsed is not None or sqanti_class is not None:
                if pbid not in match_dict:
                    print(
                        "Ignoring {0} because not on annotation (SQANTI/MatchAnnot) file.".format(
                            pbid
                        ),
                        file=sys.stdout,
                    )
                ...
                h.write(
                    "{0}\t{1}\t{2}\t".format(
                        pbid, pbid.split(".")[1], seqlen_dict[pbid]
                    )
                )
                h.write(
                    "{0}\t{1}\t{2}\t".format(
                        m["refisoform"], m["refgene"], m["category"]
                    )
                )
            else:
                h.write(
