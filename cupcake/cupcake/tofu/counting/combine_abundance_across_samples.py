...
def write_reclist_to_gff_n_info(
    rec_list, final_prefix, ref_name, addon_name, use_fq=False
):
    ...
    writer_info = DictWriter(
        f_info, fieldnames=["superPBID", ref_name, addon_name], delimiter="\t"
    )
                ...
                for _s, _e, rec_indices in cluster_tree[k][strand].getregions():
                    for i in rec_indices:
                        ...
                        for j, r in enumerate(recs):
                            f_out.write(
                                '{chr}\tPacBio\ttranscript\t{s}\t{e}\t.\t{strand}\t.\tgene_id "{gid}"; transcript_id "{gid}.{j}";\n'.format(
                                    chr=r.chr,
                                    s=r.start + 1,
                                    e=r.end,
                                    strand=strand,
                            ...
                            for exon in r.ref_exons:
                                f_out.write(
                                    '{chr}\tPacBio\texon\t{s}\t{e}\t.\t{strand}\t.\tgene_id "{gid}"; transcript_id "{gid}.{j}";\n'.format(
                                        chr=r.chr,
                                        s=exon.start + 1,
                                        e=exon.end,
                                        strand=strand,
