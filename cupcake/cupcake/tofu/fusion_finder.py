...
def pick_rep(
    ...
    for pb_id, records in record_storage.items():
        for i, r in enumerate(records):
            isoform_index = i + 1
            f_gff.write(
                '{chr}\tPacBio\ttranscript\t{s}\t{e}\t.\t{strand}\t.\tgene_id "{pi}"; transcript_id "{pi}.{j}";\n'.format(
                    chr=r.sID,
                    s=r.segments[0].start + 1,
                    e=r.segments[-1].end,
                    pi=pb_id,
                    j=isoform_index,
                    strand=r.flag.strand,
                )
            )
            for s in r.segments:
                f_gff.write(
                    '{chr}\tPacBio\texon\t{s}\t{e}\t.\t{strand}\t.\tgene_id "{pi}"; transcript_id "{pi}.{j}";\n'.format(
                        chr=r.sID,
                        s=s.start + 1,
                        e=s.end,
                        pi=pb_id,
    ...
    for pb_id in rep_info:
        best_id, best_seq, best_qual = rep_info[pb_id]
        _id_ = "{}|{}|{}".format(pb_id, coords[best_id], best_id)
        _seq_ = best_seq
        if is_fq:
            SeqIO.write(
                SeqRecord(
...
def find_fusion_candidates(
    ...
    """
    TmpRec = namedtuple(
        "TmpRec", ["qCov", "qLen", "qStart", "qEnd", "sStart", "sEnd", "iden"]
    )
    ...
    for k, data in d.items():
        ...
        if (
            len(data) > 1
            and all(a.iden >= 0.95 for a in data)
...
def fusion_main(
    ...
    print("{} fusion candidates identified.".format(count), file=sys.stderr)
    print(
        "Output written to: {0}.gff, {0}.group.txt, {1}".format(
    ...
    if cluster_report_csv is not None:
        ...
        print(
            "Count information written to: {}.abundance.txt".format(output_prefix),
            file=sys.stderr,
        )
