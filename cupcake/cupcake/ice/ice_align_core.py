...
def eval_blasr_alignment(
    ...
    if debug:
        pdb.set_trace()
    if record.qStrand == "+":
        query_qver_get_func = lambda _name, _pos: qver_get_func(
            record.qID, _name, min(_pos + record.qStart, record.qLength - 1)
        )
        query_qver_get_func1 = lambda _name, _pos: qver_get_func(
            record.qID, _name, min(_pos + record.qStart + 1, record.qLength - 1)
        )
    elif record.qStrand == "-":
        query_qver_get_func = lambda _name, _pos: qver_get_func(
            record.qID, _name, max(0, record.qEnd - 1 - _pos)
        )
        query_qver_get_func1 = lambda _name, _pos: qver_get_func(
            record.qID, _name, max(0, record.qEnd - 1 - _pos - 1)
        )
    else:
        raise Exception("Unknown strand type {}".format(record.qStrand))

    if record.sStrand == "+":
        subject_qver_get_func = lambda _name, _pos: qver_get_func(
            record.sID, _name, min(_pos + record.sStart, record.sLength - 1)
        )
        subject_qver_get_func1 = lambda _name, _pos: qver_get_func(
            record.sID, _name, min(_pos + record.sStart + 1, record.sLength - 1)
        )
    elif record.sStrand == "-":
        subject_qver_get_func = lambda _name, _pos: qver_get_func(
            record.sID, _name, max(0, record.sEnd - 1 - _pos)
        )
        subject_qver_get_func1 = lambda _name, _pos: qver_get_func(
            record.sID, _name, max(0, record.sEnd - 1 - _pos - 1)
        )
    else:
        raise Exception("Unknown strand type {}".format(record.sStrand))
    ...
    if qvmean_get_func is None:
        ...
        mean_qv_for_q = {
            "D": qvmean_get_func(record.qID, "DeletionQV"),
        ...
        if sID_starts_with_c:
            ...
            mean_qv_for_s = {
                "D": qvmean_get_func(record.sID, "DeletionQV"),
    ...
    for offset, nt_aln in enumerate(record.alnStr):
        if nt_aln == "|":  # match
            ...
            s_is_good = (
                sID_starts_with_c
                or subject_qver_get_func("InsertionQV", s_index) < mean_qv_for_s
            )
            ...
            s_is_good = (
                sID_starts_with_c
                or subject_qver_get_func1("DeletionQV", s_index) < mean_qv_for_s
            )
            ...
            if query_qver_get_func("SubstitutionQV", q_index) < mean_qv_for_q and (
                sID_starts_with_c
                or subject_qver_get_func("SubstitutionQV", s_index) < mean_qv_for_s
            ):
                ece[offset] = 1
            q_index += 1
            s_index += 1
