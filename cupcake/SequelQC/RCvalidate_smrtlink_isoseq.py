...
CAGE_BED = (
    "/home/UNIXHOME/etseng/share/FANTOM/hg38.cage_peak_phase1and2combined_coord.bed"
)
...
def collapse_to_hg38(
    ...
    if aligner_choice == "gmap":
        ...
        cmd = "{gmap} -D {gmap_db} -d {ref} -f samse -n 0 -t {cpus} -z sense_force {hq}  > {hq}.sam 2> {hq}.sam.log".format(
            gmap=smrtlink.GMAP_BIN,
            gmap_db=smrtlink.GMAP_DB,
            hq=hq_fastq,
            cpus=smrtlink.GMAP_CPUS,
            ref=ref,
        )
...
def validate_with_Gencode(out_dir, eval_dir, use_hg19_instead=False):
    ...
    os.symlink(
        os.path.join(out_dir, "touse.rep.fq.sorted.sam"), "touse.rep.fq.sorted.sam"
    )
    ...
    cmd = "python {sqanti} -t {cpus} {extra} touse.rep.fq {gtf} {genome}".format(
        ...
        extra="--cage_peak " + CAGE_BED + " -c " + INTRONPOLIS
        if not use_hg19_instead
        else "",
    )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Aligner choice (default: minimap2)",
    )
    ...
    collapse_to_hg38(
        o, a, b, args.min_count, args.aligner_choice, v, args.use_hg19_instead
    )
