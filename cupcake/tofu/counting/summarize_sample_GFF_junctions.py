...
def sanity_check(
    ...
    for d in sample_dirs.values():
        ...
        if not os.path.exists(file):
            print(
                "Expected GFF file {0} does not exist. Abort!".format(file),
                file=sys.stderr,
            )
            sys.exit(-1)
    ...
    if genome_filename is not None and not os.path.exists(genome_filename):
        print(
            "Genome file {0} given but does not exist. Abort!".format(genome_filename),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if junction_filename is not None and not os.path.exists(junction_filename):
        print(
            "Junction file {0} given but does not exist. Abort!".format(
                junction_filename
            ),
            file=sys.stderr,
        )
...
def summarize_junctions(
    ...
    f1.write(
        'track name=junctions description="{0}" useScore=1\n'.format(output_prefix)
    )
...
if __name__ == "__main__":
    ...
    if junction_filename is not None:
        print(
            "Reading junction file {0}....".format(junction_filename), file=sys.stderr
        )
