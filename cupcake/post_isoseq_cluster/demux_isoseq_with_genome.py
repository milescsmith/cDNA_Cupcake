...
def type_fafq(fafq):
    ...
    if x.endswith(".FA") or x.endswith(".FASTA"):
        ...
        raise Exception(
            "Mapped fasta/fastq filename must end with .fasta or .fastq! Saw {} instead, abort!".format(
                fafq
            )
        )
...
def link_files(src_dir, out_dir="./"):
    ...
    mapped_fasta = os.path.join(
        os.path.abspath(src_dir), "outputs", "collapse_isoforms.fasta"
    )  # SL8+ only fasta
...
def demux_isoseq_with_genome(
    ...
    print("Reading {}....".format(read_stat), file=sys.stderr)
    ...
    print("Reading {}....".format(mapped_fafq), file=sys.stderr)
    for r in SeqIO.parse(open(mapped_fafq), type_fafq(mapped_fafq)):
        m = mapped_id_rex.match(r.id)  # expected ID: PB.X.Y|xxxx.....
        if m is None:
            raise Exception("Expected ID format PB.X.Y but found {}!".format(r.id))
        pbid = m.group(1)
        f.write(pbid)
        for p in primer_names:
            f.write(",{}".format(info[pbid][p]))
        f.write("\n")
    f.close()
...
def main():
    ...
    parser.add_argument(
        "--mapped_fafq", help="mapped fasta/fastq (overridden by --job_dir if given)"
    )
    parser.add_argument(
        "--read_stat", help="read_stat txt (overridden by --job_dir if given)"
    )
    parser.add_argument(
