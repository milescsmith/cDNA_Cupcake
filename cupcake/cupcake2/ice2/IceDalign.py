...
class DalignerRunner(object):
    ...
    def __init__(
        ...
        self.query_dazz_handler = DazzIDHandler(
            self.query_filename, converted=query_converted, dazz_dir=dazz_dir
        )
        # target may have already been converted (if shared)
        target_converted = (
            target_converted or self.query_filename == self.target_filename
        )
        self.target_dazz_handler = DazzIDHandler(
            self.target_filename, converted=target_converted, dazz_dir=dazz_dir
        )
    ...
    def las_filename(self, i, j, k, is_forward, output_dir, switch_query_target=False):
        ...
        return op.join(
            output_dir,
            "{query_prefix}.{target_prefix}.{thread_prefix}.las".format(
                query_prefix=_query,
    ...
    def las_filenames(self):
        ...
        if not self.same_strand_only:
            ret.extend(
                self._las_filenames(is_forward=False, output_dir=self.output_dir)
            )
        return ret
    ...
    def la4ice_filename(self, i, j, k, is_forward, output_dir):
        ...
        return op.join(
            output_dir,
            self.las_filename(i, j, k, is_forward=is_forward, output_dir=output_dir)
            + ".out",
        )
    ...
    def _la4ice_filenames(self, is_forward, output_dir):
        ...
        return [
            self.la4ice_filename(
                i=i, j=j, k=k, is_forward=is_forward, output_dir=output_dir
            )
            for i, j, k in self._iter_i_j_k()
        ]
    ...
    def la4ice_filenames(self):
        ...
        if not self.same_strand_only:
            ret.extend(
                self._la4ice_filenames(is_forward=False, output_dir=self.output_dir)
            )
        return ret

    def daligner_cmd(self, i, j, min_match_len=300, sensitive_mode=False):
    ...
    def daligner_cmds(self, min_match_len=300, sensitive_mode=False):
        ...
        return [
            self.daligner_cmd(
                i=i, j=j, min_match_len=min_match_len, sensitive_mode=sensitive_mode
            )
            for i, j in self._iter_i_j()
        ]
    ...
    def daligner_scripts(self):
        ...
        return [
            op.join(self.script_dir, "daligner_{i}_{j}.sh".format(i=i, j=j))
            for i, j in self._iter_i_j()
        ]
    ...
    def la4ice_cmd(self, i, j, k, is_forward, output_dir):
        ...
        return " ".join(
            [
                prog,
                params,
                self.query_dazz_handler.dazz_filename,
    ...
    def _la4ice_cmds(self, is_forward, output_dir):
        ...
        """
        return [
            self.la4ice_cmd(i=i, j=j, k=k, is_forward=is_forward, output_dir=output_dir)
            for i, j, k in self._iter_i_j_k()
        ]
    ...
    def la4ice_scripts(self):
        ...
        ret = [
            op.join(self.script_dir, "la4ice_{i}_{j}_N{k}.sh".format(i=i, j=j, k=k))
            for i, j, k in self._iter_i_j_k()
        ]
        if not self.same_strand_only:
            ret.extend(
                [
                    op.join(
                        self.script_dir, "la4ice_{i}_{j}_C{k}.sh".format(i=i, j=j, k=k)
                    )
                    for i, j, k in self._iter_i_j_k()
                ]
            )
    ...
    def run(self, output_dir=".", min_match_len=300, sensitive_mode=False):
        ...
        daligner_cmds = self.daligner_cmds(
            min_match_len=min_match_len, sensitive_mode=sensitive_mode
        )

        logging.info(
            "Start daligner cmds " + ("using sge." if self.use_sge else "locally.")
        )
        logging.debug("CMD: " + "\n".join(daligner_cmds))
        ...
        if self.use_sge:
            ...
            failed.extend(
                local_job_runner(
                    cmds_list=daligner_cmds, num_threads=max(1, min(self.cpus / 4, 4))
                )
            )
        ...
        start_t = time.time()
        logging.info(
            "Start LA4Ice cmds " + ("using sge." if self.use_sge else "locally.")
        )
        ...
        if self.use_sge:
            ...
            failed.extend(
                local_job_runner(
                    cmds_list=la4ice_cmds, num_threads=max(1, min(self.cpus, 4))
                )
            )
    ...
    def clean_run(self):
        ...
        fs = (
            self._las_filenames(
                is_forward=True, output_dir=self.output_dir, switch_query_target=False
            )
            + self._las_filenames(
                is_forward=False, output_dir=self.output_dir, switch_query_target=False
            )
            + self._las_filenames(
                is_forward=True, output_dir=self.output_dir, switch_query_target=True
            )
            + self._las_filenames(
                is_forward=False, output_dir=self.output_dir, switch_query_target=True
            )
...
def add_ice_daligner_arguments(parser):
    ...
    parser.add_argument(
        "--same_strand_only", default=False, action="store_true", help=helpstr
    )
...
class IceDalignerRunner(PBToolRunner):
    ...
    def run(self):
        """ Call DalignerRunner """
        logging.info(
            "Running {f} v{v}.".format(f=op.basename(__file__), v=self.getVersion())
        )
        args = self.args
        ...
        obj = DalignerRunner(
            query_filename=args.query_fasta,
