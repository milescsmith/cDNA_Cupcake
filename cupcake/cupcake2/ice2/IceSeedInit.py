...
from cupcake.cupcake2.ice2.preCluster import preClusterSet
...
def init_seed_S(
    ...
    for r in MiniReader(minimap_filename):
        if r.qID == r.sID or r.strand == "-":
            continue  # ignore self and opp strand hits
        if r.qID > r.sID:
            continue  # ignore redundant hits since this is all-for-all
        stat = r.characterize(
            max_missed_5_len, max_missed_5_ratio, max_missed_3_len, max_missed_3_ratio
        )
