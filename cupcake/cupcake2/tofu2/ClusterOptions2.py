...
class SgeOptions2(object):
    ...
    def qsub_cmd(
        ...
        ret += "-pe {p} {n} ".format(p=self.sge_env_name, n=num_threads)
    ...
    def cmd_str(
        self,
        show_blasr_nproc=False,
        show_gcon_nproc=False,
        show_arrow_nproc=False,
        show_sge_queue=False,
        show_sge_env_name=False,
        show_qsub_extra=False,
    ):
        """Return a cmd string."""
        cmd = ""
...
class IceOptions2(object):
    ...
    def __init__(
        self,
        cDNA_size="under1k",
        flnc_reads_per_split=20000,
        ece_penalty=1,
        ece_min_len=20,
        bestn=100,
        max_missed_start=100,
        max_missed_end=30,
        ...
        self.min_match_len = (
            min_match_len  # by default, min_match_len (used by aligners) is 50 bp
        )
    ...
    def detect_cDNA_size(self, fasta_filename):
        ...
        """
        if not (
            fasta_filename.lower().endswith(".fasta")
            or fasta_filename.lower().endswith(".fa")
            or fasta_filename.lower().endswith(".xml")
        ):
            raise ValueError(
                "%s must take a FASTA ContigSet file as input." % fasta_filename
            )
        ...
        logging.info(
            "Auto-detection of cDNA size set to {n} (maxScore: {s})".format(
                n=self.cDNA_size, s=self.maxScore
            )
        )
    ...
    def sensitive_mode(self):
        ...
        """
        return self.low_cDNA_size is None or self.low_cDNA_size >= 6000
    ...
    def maxScore(self):
        ...
        if self.cDNA_size not in IceOptions2.cDNA_sizeBins():
            raise ValueError("Invalid cDNA size: {cs}".format(cs=self.cDNA_size))
        d = {
            "under500bp": -500,
            "under1k": -1000,
            "between1k2k": -2000,
    ...
    def __str__(self):
        return (
            ...
            if self.low_cDNA_size is None
            else "low={l}\nsensitive={s}\n".format(
                l=self.low_cDNA_size, s=self.sensitive_mode
            )
            + ""
            if self.high_cDNA_size is None
            else "high={h}\n".format(h=self.high_cDNA_size)
...
class IceArrowHQLQOptions2(object):
    ...
    def cmd_str(self):
        """Return a cmd string."""
        cmd = (
            "--hq_arrow_min_accuracy={n} ".format(n=self.hq_arrow_min_accuracy)
