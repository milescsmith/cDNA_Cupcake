...
def create_seed_n_batch_files(
    ...
    good = [
        x[0]
        for x in lens[starting_seed_index : starting_seed_index + num_seqs_per_batch]
    ]
    ...
    while starting_index < n:
        write_seqids_to_fasta(
            [x[0] for x in lens[starting_index : starting_index + num_seqs_per_batch]],
            "{}{}.fasta".format(batch_pre, batch_index),
            fasta_d,
        )
        starting_index += num_seqs_per_batch
        batch_index += 1
        batch_files.append("{}{}.fasta".format(batch_pre, batch_index))

    write_seqids_to_fasta(
        [x[0] for x in lens[:starting_seed_index]],
        "{}{}.fasta".format(batch_pre, batch_index),
        fasta_d,
    )
