...
def ensure_pickle_goodness(
    pickle_filename, root_dir, flnc_filename, fasta_files_to_add, fastq_files_to_add
):
    """
    ...
    if a["fasta_filename"] != os.path.abspath(os.path.join(root_dir, "current.fasta")):
        raise Exception(
            "The pickle file {} indicates that current.fasta is not being used. ICE2 likely did not finish to a point that could be picked up.".format(
                pickle_filename
            )
        )
    ...
    if "root_dir" not in a:
        print(
            "Pickle {} missing some key-values. Fixing it.".format(pickle_filename),
            file=sys.stderr,
        )
        a["root_dir"] = root_dir
        ...
        with open(pickle_filename + ".fixed", "w") as f:
            dump(a, f)
        print(
            "Fixed pickle written to {}.fixed".format(pickle_filename), file=sys.stderr
        )
...
def check_n_fix_newids(icec_obj):
    ...
    if len(newids) == 0:
        ...
        for k, v in icec_obj["d"].items():
            if len(v) != 1:
                newids.add(k)
        print("added {} seqs to newids".format(len(newids)), file=sys.stderr)
    return newids
...
def main():
    ...
    parser.add_argument(
        "pickle_filename",
        help="Last successful pickle (ex: clusterOut/output/input.split_00.fa.pickle)",
    )
    parser.add_argument(
        "--root_dir", default="clusterOut/", help="root dir (default: clusterOut/)"
    )
    parser.add_argument(
        "--flnc_fa", default="isoseq_flnc.fasta", help="(default: isoseq_flnc.fasta)"
    )
    parser.add_argument(
        "--flnc_fq", default="isoseq_flnc.fastq", help="(default: isoseq_flnc.fastq)"
    )
    parser.add_argument(
        "--fasta_files_to_add",
        default=None,
        help="Comma-separated additional fasta files to add (default: None)",
    )
    parser.add_argument(
        "--fastq_files_to_add",
        default=None,
        help="Comma-separated additional fastq files to add (default: None)",
    )
    ...
    pickup_icec_job(
        ...
        args.flnc_fa,
        args.flnc_fq,
        args.fasta_files_to_add,
        args.fastq_files_to_add,
    )
