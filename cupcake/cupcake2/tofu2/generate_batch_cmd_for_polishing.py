...
def generate_batch_cmds_for_polishing(
    chunk_prefix, nfl_filename, subread_xml, cpus, cmd_filename
):
    ...
    for fasta in fastas:
        ...
        if os.path.exists(dirname):
            print("Directory {} already exist! Abort!".format(dirname), file=sys.stderr)
            sys.exit(-1)
    ...
    for fasta in fastas:
        ...
        f.write(
            "run_IcePartial2.py all {nfl} {p}.consensus.fasta {p}.nfl.pickle "
            "--root_dir {d} --aligner_choice=daligner --cpus={c}\n".format(
                p=dirname, nfl=nfl_filename, d=real_upath(dirname), c=cpus
            )
        )
        f.write(
            "run_IceArrow2.py all {d} --subread_xml {s} --blasr_nproc {c} --arrow_nproc {c} --hq_min_full_length_reads=2\n".format(
                d=real_upath(dirname), s=subread_xml, c=cpus
            )
        )
        f.close()
        cmd_f.write(
            "qsub -cwd -S /bin/bash -pe smp 12 -V {sh}\n".format(sh=real_upath(f.name))
        )
...
def main():
    ...
    parser = ArgumentParser(
        "Generate batch commands for running IcePartial2->IceArrow2 for each collected chunk"
    )
    parser.add_argument(
        "chunk_prefix", help="Collected chunk prefix (ex: collected_final.chunk)"
    )
    ...
    parser.add_argument(
        "--cpus", default=12, type=int, help="Number of CPUs (default: 12)"
    )
    parser.add_argument(
    ...
    generate_batch_cmds_for_polishing(
        args.chunk_prefix,
        args.nfl_filename,
        args.subread_xml,
        args.cpus,
