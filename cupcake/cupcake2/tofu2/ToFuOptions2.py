...
class BaseConstants(object):
    ...
    HQ_ARROW_MIN_ACCURACY_DESC = (
        "Minimum allowed Arrow accuracy to classify an isoform "
...
def add_sge_arguments(
    arg_parser, blasr_nproc=False, arrow_nproc=False, gcon_nproc=False
):
...
def add_ice_arguments(arg_parser, tc_parser=None):
    ...
    ice_group.add_argument(
        "--aligner_choice",
        default="daligner",
        choices=["daligner", "blasr"],
        help="Aligner choice (default: daligner)",
    )
    ice_group.add_argument(
        "--targeted_isoseq",
    ...
    ice_group.add_argument(
        ...
        type=int,
        default=100,
        help="Max missed start (default: 100 bp)",
    )
    ice_group.add_argument(
        "--max_missed_end", type=int, default=30, help="Max missed end (default: 30 bp)"
    )

    # number of flnc reads per split
    ice_group.add_argument(
        "--flnc_reads_per_split",
        type=int,
        action="store",
        default=20000,
...
def add_fofn_arguments(arg_parser, subread_xml=False, tool_contract_parser=None):
    helpstr = (
        "A FOFN of bam or dataset xml (e.g., input.fofn|bam|subreadset.xml), "
    ...
    if subread_xml is True:
        ...
        if tool_contract_parser is not None:
            tool_contract_parser.add_input_file_type(
                FileTypes.DS_SUBREADS, "subreads_fofn", "SubreadSet", helpstr
            )
...
def add_tmp_dir_argument(parser):
    ...
    """
    helpstr = (
        "Directory to store temporary files." + "(default, write to root_dir/tmp.)."
    )
    parser.add_argument(
        "--tmp_dir", default=None, type=str, dest="tmp_dir", help=helpstr
    )
    return parser
...
def add_partial_argument(parser):
    ...
    parser.add_argument(
        "--aligner_choice",
        type=str,
        choices=["blasr", "daligner"],
        default="daligner",
        help="Aligner choices (default: daligner)",
    )
    parser.add_argument(
        "--cpus", type=int, default=4, help="Number of CPUs aligner uses (default: 4)"
    )
    parser.add_argument(
        "--max_missed_start",
        type=int,
        default=400,
        help="Max missed start (NOTE: currently ignored)",
    )
    parser.add_argument(
        "--max_missed_end",
        type=int,
        default=50,
        help="Max missed end (NOTE: currently ignored)",
    )
    parser.add_argument(
        "--ece_penalty", type=int, default=1, help="ECE penalty (default: 1)"
    )
    parser.add_argument(
        "--ece_min_len", type=int, default=20, help="ECE min length (default: 20bp)"
    )
    return parser


def add_nfl_fa_argument(
    arg_parser, positional=False, required=False, tool_contract_parser=None
):
    """
    ...
    helpstr = (
        "Input non-full-length reads in FASTA or ContigSet format, used for "
    ...
    if positional is True:
        ...
        arg_parser.add_argument(
            "--nfl_fa", type=str, dest="nfl_fa", required=required, help=helpstr
        )

    if tool_contract_parser is not None:
        tool_contract_parser.add_input_file_type(
            FileTypes.DS_CONTIG, "nfl_fa", "FASTA or ContigSet file", helpstr
        )
    return arg_parser


def add_nfl_fq_argument(
    arg_parser, positional=False, required=False, tool_contract_parser=None
):
    helpstr = (
        "Input non-full-length reads in FASTQ or ContigSet format, used for "
    ...
    if positional is True:
        ...
        assert required is True or required is False
        arg_parser.add_argument(
            "--nfl_fq", type=str, dest="nfl_fq", required=required, help=helpstr
        )

    if tool_contract_parser is not None:
        tool_contract_parser.add_input_file_type(
            FileTypes.DS_CONTIG, "nfl_fq", "FASTQ or ContigSet file", helpstr
        )
    return arg_parser
...
def add_cluster_root_dir_as_positional_argument(arg_parser):
    ...
    helpstr = (
        "An directory to store temporary and output cluster files"
...
def add_cluster_summary_report_arguments(parser):
    ...
    helpstr = "CSV file to output cluster info " + "(default: *.cluster_report.csv)"
    # p1 = parser.tool_contract_parser
    # p2 = parser.arg_parser.parser
    p2 = parser

    helpstr = "TXT file to output cluster summary " + "(default: *.cluster_summary.txt)"
    p2.add_argument(
        "--summary", default=None, type=str, dest="summary_fn", help=helpstr
    )
    ...
    p2.add_argument("--report", default=None, type=str, dest="report_fn", help=helpstr)

    p2.add_argument("--pickle_fn", default=None, type=str, dest="pickle_fn")
    return parser
