...
for d in dirs:
    size = 0
    for r in SeqIO.parse(open(os.path.join(d, "ccs.fasta")), "fasta"):
        size += 1
    ...
    if os.path.exists(os.path.join(d, "phased.nopartial.NO_SNPS_FOUND")):
        ...
        rec["num_snp"] = len(
            [x for x in vcf.VCFReader(open(os.path.join(d, "phased.partial.vcf")))]
        )
...
print("Summarized results of by_loci/<dirs> to {}.".format(f.name), file=sys.stderr)
