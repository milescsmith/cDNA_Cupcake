...
LOCATION_flnc_report = (
    "tasks/pbreports.tasks.isoseq_classify-0/isoseq_classify_report.json"
)
LOCATION_flnc_pattern = "tasks/pbtranscript.tasks.classify-*/isoseq_flnc.fasta"
LOCATION_cluster_report = (
    "tasks/pbreports.tasks.isoseq_cluster-0/isoseq_cluster_report.json"
)
...
def get_subread_xml(smrtlink_dir):
    d = eval(
        os.popen("grep \"'eid_subread'\" {0}/job.stdout".format(smrtlink_dir))
        .read()
        .strip()
    )
...
def collect_runtimes(smrtlink_dir, report):
    ...
    for line in open(os.path.join(smrtlink_dir, LOCATION_smrtpipe_log)):
        ...
        if (
            ...
            and line.find("Starting worker pbccs.tasks.ccs") > 0
            and ccs_start_t is None
        ):
            raw = line.strip().split()
            ccs_start_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
        elif (
            line.startswith("[INFO]")
            and line.find("Starting worker pbtranscript.tasks.classify") > 0
            and classify_start_t is None
        ):
            raw = line.strip().split()
            classify_start_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
        elif (
            line.startswith("[INFO]")
            and line.find("Starting worker pbtranscript.tasks.cluster_bins") > 0
            and cluster_start_t is None
        ):
            raw = line.strip().split()
            cluster_start_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
        elif (
            line.startswith("[INFO]")
            and line.find("Starting worker pbtranscript.tasks.ice_partial_cluster_bins")
            > 0
            and partial_start_t is None
        ):
            raw = line.strip().split()
            partial_start_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
        elif (
            line.startswith("[INFO]")
            and line.find("Starting worker pbtranscript.tasks.ice_polish_cluster_bins")
            > 0
            and polish_start_t is None
        ):
            raw = line.strip().split()
            polish_start_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
        elif (
            line.startswith("[INFO]")
            and line.find("Starting worker pbtranscript.tasks.combine_cluster_bins") > 0
        ):
            raw = line.strip().split()
            polish_end_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
        elif (
            line.startswith("[INFO]")
            and line.find("Completed execution pbsmrtpipe") > 0
        ):
            raw = line.strip().split()
            task_end_t = datetime.datetime.strptime(
                raw[1] + " " + raw[2].split(",")[0], "%Y-%m-%d %H:%M:%S"
            )
...
def gather_all_reports(smrtlink_dir, sample, out_dir="."):
    ...
    with open("smrtlink_isoseq.report.txt", "w") as f:
        f.write(
            "Sample,Cells,ZMWs,Avg. ZMW length,Subreads,Avg. Subread length,CCS reads,Avg. CCS length,FLNC reads,FLNC %,Avg. FLNC length,FLNC Length Range,CCS runtime,Classify runtime,ICE runtime,Polish runtime,Total runtime"
        )
        f.write(",HQ isoforms,Unique isoforms,Isoform Lengths\n")
        f.write(
            "{sample},{cells},{zmw},{zmwlen},{sub},{sublen},{CCS},{CCSlen},{FLNC},{FLperc},{FLNClen},{FLNCrange},{runCCS},{runFLNC},{runICE},{runPolish},{runTotal}".format(
                sample=sample,
                cells=1,
                zmw=report["numZMW"],
                zmwlen=report["avgZMWlen"],
                sub=report["numSubread"],
                sublen=report["avgSubreadlen"],
                CCS=report["numCCSread"],
                CCSlen=report["avgCCSlen"],
                FLNC=report["numFLNC"],
                FLperc=report["numFLNC"] * 1.0 / report["numCCSread"],
                FLNClen=report["avgFLNClen"],
                FLNCrange="{0:.0f}-{1:.0f}".format(
                    np.percentile(flnc_lens, 25), np.percentile(flnc_lens, 75)
                ),
                runCCS=report["runtimeCCS"],
                runFLNC=report["runtimeFLNC"],
                runICE=report["runtimeICE"],
                runPolish=report["runtimePolish"],
        ...
        f.write(
            ",{hq},{iso},{isorange}\n".format(
                hq=report["numHQ"],
                iso=report["numIso"],
                isorange="{0:.0f}-{1:.0f}".format(
                    np.percentile(iso_lens, 25), np.percentile(iso_lens, 75)
                ),
            )
        )
