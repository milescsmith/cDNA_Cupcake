...
SIRV_GENOME = (
    "/home/UNIXHOME/etseng/projects2015/Lexogen_SIRV/ground_truth/SIRV_150601a.fasta"
)
...
def link_files(src_dir, out_dir):
    ...
    hq_fastq3 = os.path.join(
        os.path.abspath(src_dir),
        "tasks",
        "pbcoretools.tasks.bam2fastq_transcripts-0",
        "hq_transcripts.fastq",
    )
    # location for HQ fastq in IsoSeq3, cromwell
    hq_fastq4 = os.path.join(
        os.path.abspath(src_dir), "outputs", "hq_transcripts.fastq"
    )
    ...
    cluster_csv2 = os.path.join(
        os.path.abspath(src_dir),
        "tasks",
        "pbtranscript2tools.tasks.collect_polish-0",
        "report.csv",
    )
    cluster_csv3 = os.path.join(
        os.path.abspath(src_dir), "tasks", "pbcoretools.tasks.gather_csv-1", "file.csv"
    )
    cluster_csv4 = os.path.join(
        os.path.abspath(src_dir), "outputs", "polished.cluster_report.csv"
    )

    class_csv3 = os.path.join(
        os.path.abspath(src_dir), "tasks", "isoseq3.tasks.refine-0", "flnc.report.csv"
    )
...
def collapse_to_SIRV(
    out_dir, hq_fastq, cluster_csv, min_count, aligner_choice, isoseq_version
):
    ...
    os.symlink(
        collapse_prefix + ".min_fl_" + str(min_count) + ".filtered.gff", "touse.gff"
    )
...
def eval_result(eval_dir, src_dir, min_count):
    tally = defaultdict(
        lambda: []
    )  # SIRV --> list of test ids that hit it (can be redundant sometimes due to fuzzy)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_dir", default="eval", help="eval dirname (default: eval)"
    )
    parser.add_argument(
        "--min_count", type=int, default=2, help="min FL count cutoff (default:2)"
    )
    parser.add_argument(
        "--aligner_choice",
        default="star",
        choices=("gmap", "star", "minimap2"),
        help="Aligner choice (default: star)",
    )
