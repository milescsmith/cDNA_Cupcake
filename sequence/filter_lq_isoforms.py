...
def main(fastq_filename, output_filename, min_fl_count, min_exp_acc, is_flnc):
    ...
    for r in SeqIO.parse(open(fastq_filename), "fastq"):
        ...
        if (is_flnc or fl_count >= min_fl_count) and exp_acc >= min_exp_acc:
            print(
                "Including {0} into output file (FL: {1}, acc: {2}).".format(
                    r.id, fl_count, exp_acc
                ),
                file=sys.stderr,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "fastq_filename", help="LQ FASTQ filename (ex: lq_isoforms.fastq"
    )
    parser.add_argument("output_filename", help="Output FASTQ filename")
    parser.add_argument(
        "--min_fl_count", default=2, type=int, help="Minimum FL count (default: 2)."
    )
    parser.add_argument(
        "--min_exp_acc",
        type=float,
        default=0.99,
        help="Minimum predicted accuracy (default: 0.99).",
    )
    parser.add_argument(
        "--is_flnc",
        action="store_true",
        default=False,
        help="Input FASTQ is FLNC, not LQ",
    )
