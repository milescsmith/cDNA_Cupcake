...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.corrected_csv):
        print(
            "Input file {} does not exist! Abort!".format(args.corrected_csv),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if not os.path.exists(args.cluster_file):
        print(
            "Input file {} does not exist! Abort!".format(args.cluster_file),
            file=sys.stderr,
        )
        sys.exit(-1)
    ...
    if ("cell_barcode" not in reader.fieldnames) or (
        ...
        print(
            "Cluster file {} must contain 'cell_barcode' and 'cluster' fields!".format(
                args.cluster_file
            ),
            file=sys.stderr,
        )
