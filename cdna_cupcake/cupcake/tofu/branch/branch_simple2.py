...
# from cdna_cupcake.cupcake.tofu.branch.intersection_unique import IntervalTreeUnique, Interval, IntervalNodeUnique
...
class ContiVec:
    ...
    def __init__(self, size):
        ...
        self.altC_pos = np.zeros(
            size, dtype=np.int
        )  # this was .C in the original code, evidence for alternative junction
        self.altC_neg = np.zeros(size, dtype=np.int)
        self.transC = np.zeros(
            size, dtype=np.int
        )  # this is .isTrans in the original code
...
class BranchSimple:
    ...
    def parse_transfrag2contig(self, gmap_sam_records, skip_5_exon_alt=True):
        ...
        for r in records:
            for i, e in enumerate(r.segments):
                ...
                if (
                    (i != 0)
                    or (i != len(r.segments) - 1)
                    or (i == 0 and (self.strand == "-" or not skip_5_exon_alt))
                    or (
                        i == len(r.segments) - 1
                        and (self.strand == "+" or not skip_5_exon_alt)
                    )
                ):
                ...
                if (
                    ...
                    self.contiVec.altC_pos[
                        (e.end - offset - 1)
                    ] += INTINF  # adjust to 0-based
                # set .transC
    ...
    def exon_finding(self):
        ...
        """
        v = self.contiVec
        self.exons = c_branch.exon_finding(
            v.baseC,
            v.altC_neg,
            v.altC_pos,
            v.transC,
    ...
    def match_record(
        ...
        for i, e in enumerate(r.segments):
            ...
            if i == 0 and num_exons > 1:
                ...
                tolerate_l = tolerate_middle
                tolerate_r = tolerate_middle
            matches = exon_matching(
                self.exons, e, tolerate_l, tolerate_r, intervals_adjacent
            )  # ToDo: CHANGE this back to c_branch later
            if matches is None:
...
def compare_exon_matrix(m1, m2, node_d, strand, merge5, max_5_diff, max_3_diff):
    ...
    for i in range(n1):
        if l1[i] == l2[0]:
            ...
            strand == "+"
            and not merge5
            and node_d[l1[i - 1]].end != node_d[l1[i]].start
        ):
            return (
                False,
                None,
            )  # 5' end disagree, in other words m1 has an extra 5' exon that m2 does not have and merge5 is no allowed
    ...
    if (
        ...
        and strand == "+"
        and (not merge5)
        and (node_d[l1[i - 1]].end - node_d[l1[0]].start > max_5_diff)
    ):
        return False, None
    if (
        i > 0
        and strand == "-"
        and (node_d[l1[i - 1]].end - node_d[l1[0]].start > max_3_diff)
    ):
        return False, None
    ...
    if j == n1 - 1:  # check that the remaining of l2 are adjacent
        ...
        for k in range(j - i + 1, n2):
            ...
            if strand == "+" and (
                node_d[l2[k - 1]].end != node_d[l2[k]].start
                or node_d[l2[k]].end - node_d[l2[j - i]].end > max_3_diff
            ):
                return False, None
            ...
            if (
                ...
                and (
                    node_d[l2[k - 1]].end != node_d[l2[k]].start
                    or node_d[l2[k]].end - node_d[l2[j - i]].end > max_5_diff
                )
            ):
                return False, None
        ...
        for k in range(j + 1, n1):
            # case 1, but for m1
            if strand == "+" and (
                node_d[l1[k - 1]].end != node_d[l1[k]].start
                or node_d[l1[k]].end - node_d[l1[j]].end > max_3_diff
            ):
                return False, None
            # case 2, but for m1
            if (
                ...
                and (
                    node_d[l1[k - 1]].end != node_d[l1[k]].start
                    or node_d[l1[k]].end - node_d[l1[j]].end > max_5_diff
                )
            ):
                return False, None
        return True, m1
...
def trim_exon_left_to_right(m1, m2, node_d, max_distance):
    ...
    for i in range(n1):
        if l1[i] == l2[0]:
            break
        elif i > 0 and node_d[l1[i - 1]].end != node_d[l1[i]].start:
            return False, None  # condition (a)
        elif node_d[l1[i]].end - node_d[l1[0]].start > max_distance:
            return False, None  # condition (b)
    ...
    for j in range(i, n1):
        if j - i > n2 - 1:
            return (
                False,
                None,
            )  # junction not yet encountered yet no more m2 (must be one-exon only), REJECT
        # matching l1[j] with l2[j-i]
        if l1[j] != l2[j - i]:  # they must not match
            return False, None
    ...
    if node_d[l1[j]].start == node_d[l1[j - 1]].end:
        return False, None  # m1 is one-exon only
