...
class FaFqSplitter(object):
    ...
    def __str__(self):
        if self.out_fns is None or len(self.out_fns) == 0:
            return (
                "{input_fasta} ".format(input_fasta=self.input_fa_or_fq)
                + "will be splitted into files each has "
                + "{n} reads.".format(n=self.reads_per_split)
            )
        else:
            return (
                "{input_fasta} has been splitted into ".format(
                    input_fasta=self.input_fa_or_fq
                )
                + "{m} files each has {n} reads:\n".format(
                    m=len(self.out_fns), n=self.reads_per_split
                )
                + ";".join(self.out_fns)
            )
    ...
    def _out_fn(self, split_index):
        ...
        if split_index > 999:
            raise ValueError(
                "Too many splitted files to generate: number "
    ...
    def split(self, reads_in_first_split=None):
        ...
        for ridx, r in enumerate(reader):
            if (
                (split_index == 0 and ridx == reads_in_first_split)
                or (split_index > 0 and ridx % self.reads_per_split == 0)
            ) and ridx != 0:
                split_index += 1
                writer.close()
                writer = open(self._out_fn(split_index), "w")
...
def get_args():
    ...
    parser.add_argument(
        "input_fa_or_fq", type=str, help="Input fasta/fastq to be splitted."
    )
    parser.add_argument("reads_per_split", type=int, help="Reads per split.")
    ...
    parser.add_argument(
        "--is_fq", default=False, action="store_true", help="Is fastq (default: False)"
    )
...
def main():
    ...
    log.info("Running {f} v{v}.".format(f=op.basename(__file__), v=get_version()))

    splitFaFq(
        input_fa_or_fq=args.input_fa_or_fq,
