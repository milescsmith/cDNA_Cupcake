...
def collect_ice2_dirs(
    dirs_to_collect, output_fasta, output_pickle, new_cid_format="b{bin}_c{cid}"
):
    ...
    for d1 in dirs_to_collect:
        ...
        for cid in uc:
            ...
            if os.path.exists(refs[cid]):
                ...
                print(
                    "WARNING: ref file {} no longer exists. new ref files not written.".format(
                        refs[cid]
                    ),
                    file=sys.stderr,
                )
        ...
        for line in fileinput.input(fasta_file):
            if line.startswith(">"):
                ...
                outf.write(
                    ">{newcid}/{rest}".format(
                        newcid=new_cid_format.format(bin=os.path.basename(d1), cid=cid),
                        rest=rest,
                    )
                )
            else:
...
def chunk_collected_fasta_pickle(
    combined_fasta, combined_uc, combined_refs, num_chunks, chunk_prefix
):
    ...
    for i in range(num_chunks):
        ...
        with open("{}.chunk{}.consensus.fasta".format(chunk_prefix, i), "w") as f:
            for seqid in keys[_from:_to]:
                r = d[seqid]
                f.write(">{}\n{}\n".format(r.id, r.seq))
        with open("{}.chunk{}.pickle".format(chunk_prefix, i), "w") as f:
            dump(
                {
                    "uc": {k: uc[k] for k in keys[_from:_to]},
                    "refs": {k: refs[k] for k in keys[_from:_to]},
                },
                f,
            )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        "Collect ICE results (final.consensus.fasta and final.pickle)"
    )
    parser.add_argument(
        "ice_dirs_file", help="File containing list of Ice2 directories"
    )
    parser.add_argument("output_prefix", help="Output prefix (ex: final_collected)")
    parser.add_argument(
        "--num_chunks", default=10, type=int, help="Number of chunks (default: 10)"
    )
    ...
    combined_uc, combined_refs = collect_ice2_dirs(
        dirs_to_collect, output_fasta, output_pickle
    )

    chunk_collected_fasta_pickle(
        combined_fasta=output_fasta,
