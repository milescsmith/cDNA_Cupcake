...
from cdna_cupcake.cupcake2.ice2.preCluster import preClusterSet2, preCluster
from cdna_cupcake.cupcake2.tofu2.run_preCluster import detect_PCR_chimeras
    ...
    for r in DictReader(open(csv_filename), delimiter=","):
        if not header_checked:
            if "seqid" not in r or "stat" not in r:
                print(
                    "{} must have the fields 'seqid' and 'stat'! Abort".format(
                        csv_filename
                    ),
                    file=sys.stderr,
                )
