...
from cdna_cupcake.cupcake2.tofu2.ClusterOptions2 import IceArrowHQLQOptions2
...
from cdna_cupcake.cupcake2.ice2.IceUtils2 import cid_with_annotation2
from cdna_cupcake.cupcake2.ice2.IceFiles2 import IceFiles2
from cdna_cupcake.cupcake2.ice2.__init__ import ICE_ARROW_PY
...
class IceArrowPostProcess2(IceFiles2):
    ...
    def __init__(
        ...
        if str(self.hq_isoforms_fa).endswith(".contigset.xml"):
            ...
            self.hq_isoforms_fa = re.sub(
                ".contigset.xml$", ".fasta", self.hq_isoforms_fa
            )
        ...
        if str(self.lq_isoforms_fa).endswith(".contigset.xml"):
            ...
            self.lq_isoforms_fa = re.sub(
                ".contigset.xml$", ".fasta", self.lq_isoforms_fa
            )
    ...
    def get_existing_binned_arrowed_fq(self):
        ...
        """
        return glob.glob(
            os.path.join(self.arrowed_dir, self.pattern_of_expected_fq_files)
        )
    ...
    def validate_inputs(self):
        ...
        if not nfs_exists(self.nfl_all_pickle_fn):
            ...
            errMsg = (
                "Log file {f}".format(f=self.arrow_submission_run_file)
    ...
    def check_arrow_jobs_completion(self):
        ...
        self.fq_filenames = []
        self.add_log(
            "Submitted arrow jobs are at {f}:".format(f=self.arrow_submission_run_file)
        )
    ...
    def pickup_best_clusters(self):
        ...
        for cid, r in polished.items():
            qv_len = max(
                len(r.quality), len(r.quality) - self.qv_trim_5 - self.qv_trim_3
            )
            ...
            if (
                expected_acc_dict[cid] >= self.hq_arrow_min_accuracy
                and len(uc[cid]) >= self.hq_min_full_length_reads
            ):
                good.append(cid)
        ...
        if self.report_fn is not None:
            self.write_report(report_fn=self.report_fn, uc=uc, partial_uc=partial_uc2)

        self.add_log(
            "Writing hiqh-quality isoforms to {f}|fq".format(f=self.arrowed_good_fa)
        )
        self.add_log(
            "Writing low-quality isoforms to {f}|fq".format(f=self.arrowed_bad_fa)
        )
        with FastaWriter(self.arrowed_good_fa) as good_fa_writer, FastaWriter(
            self.arrowed_bad_fa
        ) as bad_fa_writer, FastqWriter(
            self.arrowed_good_fq
        ) as good_fq_writer, FastqWriter(
            self.arrowed_bad_fq
        ) as bad_fq_writer:
            for cid in polished:
                r = polished[cid]
                newname = "c{cid}/f{flnc_num}p{nfl_num}/{read_len}".format(
                    cid=cid,
                    flnc_num=len(uc[cid]),
        ...
        self.add_log(
            ...
            + "to:\n{}\n{}".format(self.arrowed_good_fa, self.arrowed_good_fq),
            level=logging.INFO,
        )
        self.add_log(
            "Low-quality Arrowed consensus written "
            + "to:\n{}\n{}".format(self.arrowed_bad_fa, self.arrowed_bad_fq),
            level=logging.INFO,
        )
        self.add_log("-" * 60, level=logging.INFO)
    ...
    def _cmd_str(self, root_dir, ipq_opts, quit_if_not_done, summary_fn, report_fn):
        """Return a cmd string ($ICE_ARROW_PY postprocess)."""
        cmd = self.prog + "{d} ".format(d=root_dir) + ipq_opts.cmd_str()
    ...
    def run(self):
        ...
        if job_stats == "DONE":
            ...
            self.add_log(
                "Has incomplete jobs. Please re-run them.", level=logging.ERROR
            )
            return -1
