...
def cid_with_annotation2(cid, expected_acc=None):
    ...
    if expected_acc is not None:
        annotations.append("expected_accuracy={:.3f}".format(expected_acc))
...
def eval_sam_alignment(record, debug=False):
    ...
    offset = 0
    for _match in re.finditer(r"(\d+)(\S)", record.cigar):
        ...
        if _type in ("M", "="):
            ...
            raise Exception("Unrecognized cigar character {}!".format(_type))
