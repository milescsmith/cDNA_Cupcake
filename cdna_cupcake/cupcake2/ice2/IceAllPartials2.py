...
class IceAllPartials2(IceFiles):
    ...
    desc = (
        "Align and assign all non-full-length reads to unpolished "
    ...
    def __init__(
        ...
        IceFiles.__init__(
            self, prog_name=self.prog_name, root_dir=root_dir, tmp_dir=tmp_dir
        )

        self.fasta_filenames, self.ref_fasta = self._validate_inputs(
            fasta_filenames=fasta_filenames, ref_fasta=ref_fasta
        )
        ...
        self.cpus = (
            cpus  # this is the number of CPUs to use per SGE job or per local job
        )

        self.add_log("Making dir for mapping noFL reads: " + self.nfl_dir)
        mkdir(self.nfl_dir)

        self.add_log("input fasta files are: " + ", ".join(self.fasta_filenames))
        self.add_log("temp pickle files are: " + ", ".join(self.pickle_filenames))
    ...
    def cmd_str(self):
        ...
        return self._cmd_str(
            fasta_filenames=self.fasta_filenames,
    ...
    def pickle_filenames(self):
        ...
        return [
            op.join(self.nfl_dir, op.basename(f) + ".partial_uc.pickle")
    ...
    def done_filenames(self):
        ...
        return [
            op.join(self.nfl_dir, op.basename(f) + ".DONE")
    ...
    def script_filenames(self):
        ...
        return [
            op.join(self.script_dir, op.basename(f) + ".partial_uc.sh")
            for f in self.fasta_filenames
        ]
    ...
    def createPickles(self):
        ...
        for idx, fa in enumerate(self.fasta_filenames):
            ...
            cmd = (
                ...
                + "--max_missed_start={} ".format(self.ice_opts.max_missed_start)
                + "--max_missed_end={} ".format(self.ice_opts.max_missed_end)
                + "--ece_penalty={} ".format(self.ice_opts.ece_penalty)
                + "--ece_min_len={} ".format(self.ice_opts.ece_min_len)
            ...
            self.add_log(
                "Writing command to script {fsh}".format(fsh=self.script_filenames[idx])
            )
            with open(self.script_filenames[idx], "w") as fsh:
                fsh.write(cmd + "\n")

            # determine elog & olog
            partial_log_fn = op.join(self.log_dir, "IcePartial.{idx}".format(idx=idx))
            ...
            if self.sge_opts.use_sge is True:
                qsub_cmd = self.sge_opts.qsub_cmd(
                    script=real_upath(self.script_filenames[idx]),
                ...
                cmd += " 1>{olog} 2>{elog}".format(
                    olog=real_upath(olog), elog=real_upath(elog)
                )
    ...
    def waitForPickles(self, pickle_filenames, done_filenames):
        ...
        self.add_log(
            "Waiting for pickles {ps} to be created.".format(
                ps=", ".join(pickle_filenames)
            ),
            level=logging.INFO,
        )
        stop = False
        ...
        while stop is not True:
            stop = all(op.exists(p) for p in pickle_filenames) and all(
                op.exists(d) for d in done_filenames
            )
            sleep_time = min(600, sleep_time * 2)
            time.sleep(sleep_time)
            self.add_log(
                "Waiting for pickles to be created: {ps}".format(
                    ps=", ".join([p for p in pickle_filenames if op.exists(p)])
                )
            )
    ...
    def run(self):
        ...
        self.waitForPickles(
            pickle_filenames=self.pickle_filenames, done_filenames=self.done_filenames
        )
        # Combine all pickles to a big pickle file: nfl_all_pickle_fn.
        self.combinePickles(
            pickle_filenames=self.pickle_filenames, out_pickle=self.nfl_all_pickle_fn
        )
...
def add_ice_all_partials_arguments(parser):
    ...
    parser.add_argument(
        "fasta_filenames",
        type=str,
        help="comma delimited fasta files of " + "splitted non-full-length reads",
    )
    parser.add_argument(
        "ref_fasta",
        type=str,
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="comma delimited fastq files of " + "splitted non-full-length reads",
    )
    parser.add_argument(
        "--root_dir",
        dest="root_dir",
    ...
    parser = add_sge_arguments(
        parser, blasr_nproc=False, arrow_nproc=False, gcon_nproc=False
    )
    ...
    parser.add_argument(
        ...
        dest="done_filename",
        type=str,
        help="An empty file generated to indicate that " + "out_pickle is done.",
    )
