...
from cupcake.cupcake2.tofu2.ice_pbdagcon2 import runConsensus
...
class IceIterative2(IceFiles2):
    ...
    def __init__(
        ...
        with ContigSetReaderWrapper(fasta_filename) as cs:
            self.newids.update({r.id for r in cs})
    ...
    def run_gcon_parallel_helper(self, cids):
        ...
        if effective_i > 0:
            self.add_log("use_sge = {}".format(self.use_sge))
    ...
    def choose_ref_file(self, cid):
        ...
        if op.exists(cons) and os.stat(cons).st_size > 0:
            ...
            self.add_log("Neither {} nor {} exists!.".format(cons, cons_ref))
        ...
        for cid in cids:
            ...
            for qid in self.uc[cid]:
                if (
                    ...
                    msg = "Final round: remove {} (from {}) because {}".format(
                        qid, cid, self.d[qid]
                    )
                    self.add_log(msg)
                    del self.d[qid]
    ...
    def add_seq_to_cluster(self):
        ...
        for sid in self.newids:
            if len(self.d[sid]) == 0:  # no match to existing cluster
                ...
                cid = best[0][0]
                r = self.seq_dict[sid]
                msg = "adding {} to c{}".format(sid, cid)
                self.add_log(msg)
        ...
        for qID in self.d:
            ...
            if len(x) == 0:
                ...
                if best_i != qid_to_cid[qID]:
                    ...
                    msg = "best for {} is {},{} (currently: {}, {})".format(
                        qID,
                        best_i,
                        best_i_prob,
                        old_i,
                    ...
                    if len(self.uc[old_i]) <= 2:
                        ...
                        if len(x) > 0 and random.random() <= self.random_prob:
                            ...
                            best_i, best_i_prob = x[0][0], x[0][1]
                            msg = "randomly moving {} from {} to {}".format(
                                qID, old_i, best_i
                            )
                            self.add_log(msg)
    ...
    def add_new_batch(self, batch_fasta, batch_fastq):
        ...
        for cid, ids in self.uc.items():
            if len(ids) >= self.rerun_gcon_size:
                ...
                msg = "Removing from probQV all members of {}".format(cid)
                self.add_log(msg)
        ...
        with ContigSetReaderWrapper(self.fasta_filename) as reader:
            for r in reader:
                ...
                if rid not in self.newids:
                    self.newids.add(rid)
                    # has_err = False
                    msg = "{} should be in newids but not. Add it!".format(rid)
                    self.add_log(msg)
                try:
                    ...
                    msg = "{} should be in probQV but not. Add it!".format(rid)
                    self.add_log(msg)
        ...
        try:
            for cid, members in self.uc.items():
                ...
                if cid not in self.refs or len(self.refs[cid]) == 0:
                    print("ref for {} does not exist!".format(cid))
    ...
    def run_post_ICE_merging(self, consensusFa, pickleFN, max_iter, use_blasr):
        ...
        self.add_log(
            "run_post_ICE_merging called with max_iter={}, using_blasr={}".format(
                max_iter, use_blasr
            )
        )
        for _i in range(max_iter):
            self.add_log("Before merging: {} clusters".format(len(self.uc)))
            self.add_log(
            ...
            self.add_log("After merging: {} clusters".format(len(self.uc)))
    ...
    def do_icec_merge_nogcon(self, r):
        ...
        if i in self.old_rec:
            if j in self.old_rec:  # both i, j in old_rec
                ...
                k = self.old_rec[i]
                self.add_log(
                    "case 1: Merging clusters {} and {} --> {}".format(i, j, k)
                )
                self.uc[k] += self.uc[j]
            ...
            if j in self.old_rec:  # i is new, but j is old, add i to k
                k = self.old_rec[j]
                self.add_log(
                    "case 2: Merging clusters {} and {} --> {}".format(i, j, k)
                )
                self.uc[k] += self.uc[i]
                ...
                self.add_log(
                    "case 3: Merging clusters {} and {} --> {}".format(i, j, k)
                )
